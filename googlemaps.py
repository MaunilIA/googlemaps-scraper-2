...
class GoogleMapsScraper:
    ...
    def sort_by(self, url, ind):
        self.rating_flag = -1 #tells if the ratings are given by stars or simply 1/5, 2/5, 3/5, etc.
        self.driver.get(url)
        ...
        tries = 0
        try:
            all_review_btn = wait.until(EC.element_to_be_clickable((By.XPATH, '//button[@class="HQzyZ"]')))
            self.rating_flag = 1
        except:
            all_review_btn = None
        print(all_review_btn)
        while not clicked and tries < MAX_RETRY:
            if all_review_btn:
                try:
                    ...
                    print("Error: 1")
                    time.sleep(3)
                    review_btn = self.driver.find_elements_by_xpath('//div[@role=\'menuitemradio\']')[1]
                    print("Error: 2")
                    review_btn.click()
                    print("Error: 3")
                    time.sleep(3)

                    menu_bt = wait.until(EC.element_to_be_clickable((By.XPATH, "//button[@aria-label=\'Most relevant\']")))# r'/html/body/div[3]/div[9]/div[9]/div/div/div[1]/div[2]/div/div[1]/div/div/div[2]/div[9]/button[2]'
                    print("Error: 4")
                    menu_bt.click()
                    print("Error: 5")
                    time.sleep(3)
                    ...
                    tries+=1
                    self.logger.warn("Failed to click either sorting button or reivews type button")
                    print("Failed to click either sorting button or reivews type button")
            else:
                try:
    ...
    def __get_driver(self, debug=False):
        ...
        input_driver = webdriver.Chrome(executable_path=ChromeDriverManager().install(),options=options)#log_level=0
